...
In order to utilize a package and it's functions we need to do 2 things: 
    ...
    1. Install the package in the console: type install.packages("name of package you want to install")
...
If you ever receive the error message: "package name" does not exist, the fix is simple:
        - Go to the console in R Studio and type install.packages("name of package you want to install")
...
library(ggiraph) # makes static maps interactive
...
## Define API Key
[Request Here](https://api.census.gov/data/key_signup.html)
Place the API Key emailed to you in the code below replacing 06f9dcf5172cd1b403f9a6c34beea0d7929604f3 with your own
...
The dataframe we just created (acs_2022_variables) has all the info we need, but is pretty bland
Let's create a table that is prettier and easier to navigate 
- works best when knitted
